...
| Kotlin Gradle Plugin warning from `mobile_scanner` + `speech_to_text` — future Flutter releases will refuse to build apps whose plugins apply KGP. Both are already on their newest release (7.4.0) and neither upstream has migrated to Built-in Kotlin, so there is nothing to upgrade to today; re-check before any Flutter upgrade | owner-reported 2026-08-23 | — | before the next Flutter upgrade |
| ~~Invoice issuance answered 500 for a workshop whose VAT number was 15 digits but ended in 9~~ ✅ 2026-08-23 — found by the seam walk, not by tests. The DTO guard was bypassed because the seed writes straight to the database, so the value exploded at the deepest point (the ZATCA QR encoder) with «حدث خطأ غير متوقع» — at the exact moment a workshop asks for its money. Fixed at issuance with an Arabic message that says what to do, pinned by a regression test, seed corrected and four dev orgs swept. **Lesson: a guard enforced in one layer is not a guard** — a DB CHECK on the column was proposed so no seed, tool or manual UPDATE can write it again | live seam walk 2026-08-23 | — | done (CHECK constraint proposed to the API session) |
| ~~Marketplace seam: the workshop could neither see requests nor answer~~ ✅ 2026-08-23 — `?nearby=true` returned an empty list while the API keyed on `org_id`, and `offer` demanded an `org_id` the app never sent; the supply side of the flagship market was dead SILENTLY (an empty list looks like «no requests yet»). Both sides fixed, and better than asked: a manager of two branches now sees both, and an offer infers its organization from the request itself | live three-sided walk 2026-08-23 | — | done |
| ~~Funnel stages counted independently — «106.5% نال عرضاً»~~ ✅ 2026-08-23 — a rate above 100% in an ops dashboard voids trust in every number beside it; stages now resolve from the source tables for a cohort defined by events, so they shrink by construction (44 → 26 → 25) and a test asserts monotonicity, not one reading | live three-sided walk 2026-08-23 | — | done |
| ~~Bid with photos (supplier attaches media to a bid)~~ ✅ 2026-08-23 — up to 6 photos ride a bid through the generic `media_links` (no schema change); the compare view carries `media_ids`, and the photo opens for the bidding supplier and the comparing buyer only (403 for anyone else, pinned by e2e) | Step 22 | — | done |
...
| ~~Repair request from a carless customer~~ ✅ 2026-08-23 — it was worse than a gap: `vehicle_id` is REQUIRED by the API, so the pilot's very first user hit a bare validation error on «أرسل الطلب». The sheet now shows «أضف سيارتك أولاً» **instead of** the send button and the rest of the form (never fill a form that will be refused); the API requirement stands on purpose (contract corrected in the scope doc §7). Three existing tests had been enshrining the broken behaviour — found by a live walk, not by tests (b79996d) | live product walk 2026-08-23 | — | done |
...
- Organizations: لا مسار «اجعل هذا الفرع رئيسياً» (`PATCH /organizations/:id/locations/:locId`) — ورشة بفرعين لا تستطيع تصحيح فرعها الرئيسي بنفسها؛ ظهرت الحاجة في مشي حلقة القطع 2026-08-28 (المطابقة دارت حول الفرع الخطأ).

## ملصقات QR وحركة الشاشات (٢٣ سبتمبر ٢٠٢٦)

- **ملصقات القطع للتشليح والمحل** شُحنت بأصغر شكلٍ يُكمل الحلقة: القطعة تُسجَّل باسم المنشأة
  (`POST /parts/orgs/:id/catalog` بعلامة = اسمها) فيصبح لها حقّ إصدار الأرقام، ثم دفعة QR،
  ثم شاشة `PartLabelsScreen` (رمز + رقم تسلسلي لكل قطعة). **متبقٍ**: مشاركة/طباعة الدفعة كـPDF
  أو صورة (يحتاج `printing`/`share_plus`)، وربط الملصق بالعرض في المزاد عند البيع.
- **حركة الشاشات**: لم تُضَف انتقالات مخصّصة عمداً — ميثاق البساطة §5.0/6 «لا حركة زخرفية».
  Material 3 على Flutter 3.47 يعطي انتقال fade-forwards افتراضياً، والحركة الموجودة كلها حركة حالة
  (`AnimatedContainer` لاختيار العرض، الرادار على الخريطة، `AnimatedSwitcher` في الرئيسية).
- **السائق على الجهاز — مُشي (٢٣ سبتمبر)**: TJ-2026-000076 من القبول إلى التسليم والفاتورة؛ أربعة
  أعطال أُصلحت في `5c512bf` (كاميرا الإثبات، غرض الرفع، عنوان الرفع الحلقي، تأكيد القبول).
- **الأسطول على الجهاز — مُشي (٢٣ سبتمبر)**: الدخول، «أسطولك اليوم»، قواعد الصرف (حُفظت 500/3000/20000
  وظهرت على الرئيسية). **فجوتان**: لا شاشة **استيراد قائمة المركبات** في التطبيق مع أن
