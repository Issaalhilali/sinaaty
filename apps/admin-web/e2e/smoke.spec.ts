...
test('login → overview → KYB → integrations', async ({ page }) => {
  ...
  await page.getByRole('link', { name: 'المنشآت', exact: true }).click(); await expect(page.getByRole('heading', { name: 'المنشآت و KYB' })).toBeVisible();
  ...
  await page.getByRole('link', { name: 'الربط مع الجهات' }).click(); await expect(page.getByRole('heading', { name: 'التكاملات' })).toBeVisible();
  await page.getByRole('link', { name: 'النزاعات', exact: true }).click(); await expect(page.getByRole('heading', { name: 'غرفة النزاعات' })).toBeVisible();
