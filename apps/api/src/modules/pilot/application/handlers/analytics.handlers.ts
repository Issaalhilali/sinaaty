...
export class AnalyticsOutboxHandlers implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    on('ServiceOfferSubmitted', 'service-offered', (ev) => ({ event: 'service_offer.submitted', orgId: str(ev.payload['orgId']), entityType: 'service_offer', entityId: str(ev.payload['offerId']) ?? ev.aggregateId, props: { request_id: ev.aggregateId, offer_type: str(ev.payload['offerType']), price_min: money(ev.payload['priceMin']) } }));
