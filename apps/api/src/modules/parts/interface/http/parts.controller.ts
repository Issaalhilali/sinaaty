...
export class PartsController {
  ...
  @Public() @Get('verify/:token') @ApiOperation({ summary: 'Public genuine-part check by QR token' }) verify(@Param('token') token: string) { return this.serials.verify(token); }
  // ---- warranties
  ...
  @Public() @Get('warranty-check/:token') warrantyCheck(@Param('token') token: string) { return this.warranties.publicByToken(token); }
  // ---- group buys
