...
export class SerialsUseCases {
  ...
  async verify(qrToken: string) {
    const s = await this.repo.findSerialByToken(qrToken); if (!s) return { genuine: false, message_ar: 'هذا الرمز غير مسجّل — القطعة غير موثّقة عبر صناعتي.', message_en: 'Unknown code — this part is not Sinaaty-verified.' };
