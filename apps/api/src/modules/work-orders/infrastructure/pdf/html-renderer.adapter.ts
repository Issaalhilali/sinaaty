...
<div class="foot">بصمة المستند (SHA-256): <code>${esc(o.signatureRef ?? '')}</code>${o.signed && o.signedAt ? `<br>وُقّع في ${esc(o.signedAt.toLocaleString('ar-SA', { timeZone: 'Asia/Riyadh' }))} · شروط العقد ${esc(s.contract_terms_version)}` : ''}<br>صناعتي — منصة موثّقة لسوق إصلاح السيارات</div>
</body></html>`;
