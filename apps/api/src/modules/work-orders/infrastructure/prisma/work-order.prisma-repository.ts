...
const woInclude = {
  ...
  org: { select: { tradeNameAr: true, legalNameAr: true } },
...
export class WorkOrderPrismaRepository implements WorkOrderRepository {
  ...
  async listVersions(woId: string) {
    // طريقة التوقيع تسافر مع النسخة: الشاشة تقول «موقّع بنفاذ» أو «موقّع برمز التحقق» بحسب ما جرى فعلاً،
    // لا بحسب ما نتمنّاه — الادّعاء بنفاذ فوق توقيع برسالة نصية كذبٌ على مستند قانوني.
    const rows = await this.prisma.workOrderVersion.findMany({ where: { workOrderId: woId }, orderBy: { version: 'asc' }, select: { id: true, version: true, snapshotSha256: true, reasonAr: true, createdAt: true, workOrderSignatures: { select: { method: true }, orderBy: { signedAt: 'asc' } } } });
    return rows.map((r) => ({ id: r.id, version: r.version, sha256: r.snapshotSha256, reasonAr: r.reasonAr, createdAt: r.createdAt, signed: r.workOrderSignatures.length > 0, signedMethod: r.workOrderSignatures[0]?.method ?? null }));
  }
