...
export interface NewInvoice { orgId: string; number: string; type: InvoiceType; status: InvoiceStatus; workOrderId?: string; parentInvoiceId?: string; customerUserId?: string; customerOrgId?: string; buyerSnapshot: PartySnapshot; sellerSnapshot: PartySnapshot; subtotal: string; discountTotal: string; vatTotal: string; total: string; paymentTerms: PaymentTerms; issueDate: Date; dueDate?: Date | null; supplyDate?: Date | null; zatcaUuid: string; zatcaHash: string; zatcaQr: string; zatcaStatus: ZatcaStatus; notesAr?: string; createdBy: string; lines: NewInvoiceLine[] }
