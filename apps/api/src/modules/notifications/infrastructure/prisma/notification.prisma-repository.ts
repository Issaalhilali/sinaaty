...
export class NotificationPrismaRepository implements NotificationRepository {
  ...
  async userContact(userId: string): Promise<{ phone: string | null; locale: 'ar' | 'en' } | null> { const u = await this.prisma.user.findUnique({ where: { id: userId }, select: { phoneE164: true, preferredLocale: true } }); return u ? { phone: u.phoneE164, locale: (u.preferredLocale === 'en' ? 'en' : 'ar') } : null; }
}
