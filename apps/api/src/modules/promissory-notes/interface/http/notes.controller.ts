...
export class NotesController {
  ...
  @Get('promissory-notes') @ApiOperation({ summary: 'List: ?org_id= as creditor (workshop), else my notes as debtor; ?status=a,b&overdue=true' })
  list(@CurrentUser() u: AuthUser, @Query('org_id') orgId?: string, @Query('status') status?: string, @Query('overdue') overdue?: string, @Query('limit') limit?: string) { return this.uc.list(u, { org_id: orgId, status: status ? (status.split(',') as PnStatus[]) : undefined, overdue: overdue === 'true', limit: limit ? Number(limit) : undefined }); }
