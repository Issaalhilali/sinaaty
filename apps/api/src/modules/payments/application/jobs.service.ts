...
import { ApprovalsUseCases } from './use-cases/approvals.use-cases';
...
export class PaymentJobs {
  ...
  constructor(private readonly escrow: EscrowUseCases, private readonly wallet: WalletUseCases, private readonly approvals: ApprovalsUseCases, private readonly config: AppConfig, private readonly lock: LeaderLock) {}
  ...
  @Interval(6 * 60 * 60_000) async expireApprovals() { if (!this.config.get('JOBS_ENABLED')) return; const r = await this.lock.runExclusive('approvals.expire', () => this.approvals.expireDue()); if (r?.expired) this.log.log(`expired ${r.expired} unanswered approval requests`); }
