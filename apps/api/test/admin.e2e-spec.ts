...
describe('Admin back-office API (e2e)', () => {
  ...
  beforeAll(async () => { const mod = await Test.createTestingModule({ imports: [AppModule] }).compile(); app = mod.createNestApplication(); app.enableVersioning({ type: VersioningType.URI, defaultVersion: '1' }); await app.init(); admin = await login('+966500000099'); ws = await login('+966500000001'); });
  ...
  it('audit browser: filter + CSV export', async () => {
    const r = await http().get('/v1/admin/audit?action=invoice.issue&limit=5').set(auth(admin)).expect(200); expect(r.body.length).toBeGreaterThan(0); expect(r.body[0].action).toContain('invoice.issue'); expect(r.body[0].hash).toHaveLength(64);
    const csv = await http().get('/v1/admin/audit.csv?action=invoice.issue').set(auth(admin)).expect(200); expect(csv.headers['content-type']).toMatch(/text\/csv/); expect(csv.text.split('\n')[0]).toContain('id,occurred_at,actor_user_id');
  });
  ...
  it('integrations monitor lists requests/webhooks/dead letters; retry needs a reason and is audited', async () => {
    ...
    const esc = await http().get('/v1/admin/escrow').set(auth(admin)).expect(200); expect(esc.body[0]).toHaveProperty('beneficiaryNameAr');
  });
