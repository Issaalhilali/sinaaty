...
class FakeParts implements PartsRepository, QrScanner {
  ...
  @override Future<Result<PartRequest>> createRequest({String? orgId, String? workOrderId, String? vin, required String partNameAr, String? descriptionAr, List<String>? acceptedConditions, int quantity = 1, int? biddingMinutes, double? lat, double? lng}) async { seq++; final r = PartRequest(id: 'pr$seq', number: 'PR-2026-00080$seq', partNameAr: partNameAr, vin: vin, acceptedConditions: acceptedConditions ?? ['oem_new'], quantity: quantity, biddingEndsAt: DateTime.now().add(Duration(minutes: biddingMinutes ?? 60)), status: 'open', requesterOrgId: orgId, createdAt: DateTime.now()); requests[r.id] = r; return Result.ok(r); }
