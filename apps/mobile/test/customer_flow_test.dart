...
class FakeWorkOrders implements WorkOrdersRepository, WorkOrderRealtime {
  ...
  /// يحاكي ضياع الردّ **بعد** أن يسجّل الخادم التوقيع — الحالة تتقدّم والعميل لا يعلم.
  bool loseReplyAfterSigning = false;
  ...
  @override Future<Result<WorkOrder>> approveComplete(String id, {required String method, int? version, String? transactionId, String? code}) async {
    lastCode = code;
    if (method == 'otp' && code != '123456') return const Result.err(ApiFailure(400, 'OTP_INVALID', 'رمز غير صحيح', 'Invalid code'));
    status = 'in_progress';                                   // الخادم سجّل التوقيع فعلاً
    if (loseReplyAfterSigning) return const Result.err(NetworkFailure());   // ثم ضاع الردّ
    return Result.ok(wo);
  }
...
class FakeVehicles implements VehiclesRepository {
  @override Future<Result<List<Vehicle>>> list({String? orgId}) async => const Result.ok([Vehicle(id: 'v1', vin: 'JTDKN3DU0A0123456', plate: 'أ ب ج 4821', makeAr: 'تويوتا', modelAr: 'كامري', year: 2019, odometerKm: 84250)]);
...
void main() {
  ...
  testWidgets('ضاع الردّ بعد التوقيع: الشاشة تسأل الخادم فتقول «تم الاعتماد» لا خطأً', (tester) async {
    tester.view.physicalSize = const Size(1170, 2532); tester.view.devicePixelRatio = 3; addTearDown(tester.view.reset);
    wos.loseReplyAfterSigning = true;
    await tester.pumpWidget(app(router('/work-orders/wo1/approve'))); await tester.pumpAndSettle();
    await tester.ensureVisible(find.text('اعتماد برمز التحقق')); await tester.tap(find.text('اعتماد برمز التحقق')); await tester.pumpAndSettle();
    await tester.scrollUntilVisible(find.byType(TextField), 200, scrollable: find.byType(Scrollable).first);
    await tester.enterText(find.byType(TextField), '123456');
    await tester.tap(find.text('تأكيد الاعتماد')); await tester.pumpAndSettle();
    expect(wos.status, 'in_progress');                                   // التوقيع وقع فعلاً
    expect(find.text('تم الاعتماد — الورشة تبدأ العمل'), findsOneWidget); // فلا يُقال له إنه فشل
