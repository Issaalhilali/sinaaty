...
Future<void> _approve(BuildContext context, WidgetRef ref, TradeAccount a) async {
    ...
    r.when(ok: (_) => ref.invalidate(sellerTradeAccountsProvider), err: (f) => ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(f.message(Localizations.localeOf(context).languageCode)))));
  }
Future<void> _issueSerials(BuildContext context, WidgetRef ref) async {
    ...
    r.when(ok: (b) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(l.spSerialsIssued(b.issued, b.batchCode)))); context.push('/sp/labels/${b.batchCode}?org=$org'); }, err: (f) => ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(f.message(Localizations.localeOf(context).languageCode)))));
  }
...
Future<void> _makeLabels(BuildContext context, WidgetRef ref) async {
    ...
    r.when(ok: (b) => context.push('/sp/labels/${b.batchCode}?org=$org&name=${Uri.encodeComponent(name.text.trim())}'), err: (f) => ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(f.message(Localizations.localeOf(context).languageCode)))));
  }
