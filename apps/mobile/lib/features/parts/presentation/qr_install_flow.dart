...
Future<void> startQrInstall(BuildContext context, WidgetRef ref, {String? token}) async {
  ...
  await v.when(err: (f) async => ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(f.message(locale)))), ok: (sv) async {
    ...
    final go = await showModalBottomSheet<bool>(context: context, showDragHandle: true, isScrollControlled: true, builder: (ctx) => StatefulBuilder(builder: (ctx, setS) => Padding(padding: EdgeInsets.fromLTRB(SinaatySpace.lg, 0, SinaatySpace.lg, MediaQuery.viewInsetsOf(ctx).bottom + SinaatySpace.xl), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      SealCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Icon(sv.genuine ? Icons.verified : Icons.gpp_bad_outlined, color: Colors.white), const SizedBox(width: 8), Expanded(child: Text(sv.genuine ? l.ptVerifyGenuine : l.ptVerifyFake, style: Theme.of(ctx).textTheme.titleLarge?.copyWith(color: Colors.white))), if (sv.alert) SealPill(l.ptVerifyAlert, icon: Icons.warning_amber_rounded)]), if (sv.partNameAr != null) Text('${sv.partNameAr} · ${sv.partNumber ?? ''} · ${sv.issuerAr ?? ''}', style: TextStyle(color: Colors.white.withValues(alpha: .85))), Text(locale == 'ar' ? sv.messageAr : sv.messageEn, style: TextStyle(color: Colors.white.withValues(alpha: .8), fontSize: 13))])),
      if (sv.genuine && sv.status != 'installed') ...[
        ...
        RadioGroup<String>(groupValue: itemId, onChanged: (v) => setS(() => itemId = v), child: Column(children: [for (final w in orders) for (final i in w.items.where((i) => i.type == 'part')) RadioListTile<String>(value: i.id, title: Text(i.descriptionAr), subtitle: Text('${w.titleAr ?? ''} · ${w.number}'), contentPadding: EdgeInsets.zero, dense: true)])),
