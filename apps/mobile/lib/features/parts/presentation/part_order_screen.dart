...
class _PartOrderScreenState extends ConsumerState<PartOrderScreen> {
  ...
  @override Widget build(BuildContext context) {
    ...
    final disputesOn = (ref.watch(featureFlagsProvider(org)).value ?? FeatureFlags.allVisible).enabled(Flags.disputes);
    final dispute = !disputesOn ? null : (ref.watch(disputes.myDisputesProvider).value?.valueOrNull ?? const []).where((d) => d.partOrderId == widget.id && d.live).firstOrNull;
    final canDispute = disputesOn && dispute == null && o != null && const {'paid', 'preparing', 'shipped', 'delivered', 'installed'}.contains(o.status);
    return AppScaffold(title: o?.number ?? l.ptMyOrders, primaryAction: primary,
      moreItems: canDispute ? [PopupMenuItem(value: 'dispute', child: Text(l.dsOpen))] : null,
      onMore: (val) { if (val == 'dispute') openDisputeSheet(context, ref, partOrderId: widget.id); },
      body: AsyncResultView<PartOrder>(value: v, onRetry: _refresh, builder: (o) => ListView(padding: const EdgeInsets.fromLTRB(SinaatySpace.lg, SinaatySpace.sm, SinaatySpace.lg, 96), children: [
        if (dispute != null) ...[
