...
class _WorkshopPartsScreenState extends ConsumerState<WorkshopPartsScreen> {
  ...
  Future<void> _buy(PartOffer o) async {
    ...
    final ok = await showModalBottomSheet<bool>(context: context, showDragHandle: true, isScrollControlled: true, builder: (ctx) => StatefulBuilder(builder: (ctx, setS) => Padding(padding: const EdgeInsets.fromLTRB(SinaatySpace.lg, 0, SinaatySpace.lg, SinaatySpace.xl), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      Row(children: [Expanded(child: Text(l.ptQty, style: Theme.of(ctx).textTheme.titleSmall)), IconButton(onPressed: qty > 1 ? () => setS(() => qty--) : null, icon: const Icon(Icons.remove_circle_outline)), Text('$qty', style: Theme.of(ctx).textTheme.titleLarge), IconButton(onPressed: qty < o.quantity ? () => setS(() => qty++) : null, icon: const Icon(Icons.add_circle_outline))]),
  ...
  Future<void> _auction() async {
    ...
    final ok = await showModalBottomSheet<bool>(context: context, showDragHandle: true, isScrollControlled: true, builder: (ctx) => StatefulBuilder(builder: (ctx, setS) => SheetBody(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      const SizedBox(height: SinaatySpace.md), Row(children: [Expanded(child: Text(l.ptBiddingMinutes, style: Theme.of(ctx).textTheme.titleSmall)), SegmentedButton<int>(segments: const [ButtonSegment(value: 30, label: Text('30')), ButtonSegment(value: 60, label: Text('60')), ButtonSegment(value: 240, label: Text('240'))], selected: {minutes}, onSelectionChanged: (s) => setS(() => minutes = s.first), showSelectedIcon: false)]),
  ...
  @override Widget build(BuildContext context) {
    ...
    return RefreshIndicator(onRefresh: () async { ref.invalidate(buyerTradeAccountsProvider); ref.invalidate(myPartRequestsProvider); ref.invalidate(myPartOrdersProvider); }, child: ListView(padding: EdgeInsets.fromLTRB(SinaatySpace.lg, SinaatySpace.sm, SinaatySpace.lg, SinaatySpace.bottomClearance(context)), children: [
      SectionTitle(l.ptSearchByVin),   // one entry point per action: QR lives in «أدواتي» below
      ...
      // الحيّ أولاً بالأقرب انتهاءً، ثم ذيلٌ قصير من المنتهي. `take(5)` على القائمة كما جاءت كان
      // يعرض الأحدث — وخمسة منتهية تدفن المزاد الذي ما زال يقبل عروضاً، فتُقرأ الشاشة مقبرةً.
      if (reqs.isNotEmpty) ...[const SizedBox(height: SinaatySpace.lg), SectionTitle(l.ptMyRequests), SectionCard(padding: const EdgeInsets.symmetric(horizontal: SinaatySpace.sm), child: RowGroup(children: [for (final r in supplierInbox(reqs, deadTail: 2).take(6)) AppListRow(icon: Icons.gavel_outlined, title: r.partNameAr, subtitle: r.open ? Fmt.meta([r.number, Labels.endsIn(l, r.remaining)]) : (r.bidsCount == 0 ? l.ptEndedNoBids : Fmt.meta([r.number, l.ptEnded])), trailing: StatusBadge(r.bidsCount > 0 ? l.ptBidsCount(r.bidsCount) : (r.open ? l.ptNoBidsYet.split(' —').first : l.ptEnded), tone: r.bidsCount > 0 ? BadgeTone.brass : BadgeTone.plain), onTap: () => context.push('/parts/requests/${r.id}'))]))],
      if (orders.isNotEmpty) ...[const SizedBox(height: SinaatySpace.lg), SectionTitle(l.ptMyOrders), SectionCard(padding: const EdgeInsets.symmetric(horizontal: SinaatySpace.sm), child: RowGroup(children: [for (final o in orders.take(6)) AppListRow(icon: Icons.inventory_2_outlined, title: o.items.firstOrNull?.descriptionAr ?? o.number, subtitle: Fmt.meta([if (o.items.firstOrNull?.descriptionAr != null) o.number, Fmt.money(o.total, locale: locale)]), trailing: StatusBadge(Labels.partOrderStatus(l, o.status), tone: o.status == 'delivered' ? BadgeTone.brass : o.isActive ? BadgeTone.seal : BadgeTone.plain), onTap: () => context.push('/parts/orders/${o.id}'))]))],
