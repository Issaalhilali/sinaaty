...
Vehicle vehicleFromJson(Map<String, dynamic> j) => Vehicle(id: j['id'] as String, vin: j['vin'] as String?, plate: j['plateNumber'] as String?, makeAr: j['makeNameAr'] as String?, modelAr: j['modelNameAr'] as String?, year: (j['modelYear'] as num?)?.toInt(), colorAr: j['colorAr'] as String?, odometerKm: (j['odometerKm'] as num?)?.toInt(), makeId: (j['makeId'] as num?)?.toInt(), lastServiceAt: j['last_service_at'] == null ? null : DateTime.parse(j['last_service_at'] as String), lastServiceTitleAr: j['last_service_title_ar'] as String?, activeWarranties: (j['active_warranties'] as num?)?.toInt() ?? 0, openWorkOrderId: j['open_work_order_id'] as String?);
class VehiclesRepositoryImpl implements VehiclesRepository {
  final ApiClient api; VehiclesRepositoryImpl(this.api);
  @override Future<Result<List<Vehicle>>> list({String? orgId}) async { try { final r = await api.dio.get<List<dynamic>>('/vehicles', queryParameters: {'org_id': ?orgId}); return Result.ok(r.data!.map((e) => vehicleFromJson(e as Map<String, dynamic>)).toList()); } catch (e) { return Result.err(mapDioError(e)); } }
  @override Future<Result<Vehicle>> add({String? vin, String? plate, String? ownerOrgId}) async { try { final r = await api.dio.post<Map<String, dynamic>>('/vehicles', data: {'owner_org_id': ?ownerOrgId, if (vin != null && vin.isNotEmpty) 'vin': vin, if (plate != null && plate.isNotEmpty) 'plate': plate}); return Result.ok(vehicleFromJson(r.data!)); } catch (e) { return Result.err(mapDioError(e)); } }
