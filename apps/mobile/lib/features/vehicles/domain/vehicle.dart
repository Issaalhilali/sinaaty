class Vehicle {
  ...
  String get title => [makeAr, modelAr, year?.toString()].whereType<String>().join(' ').trim().isEmpty ? (plate ?? vin ?? '') : [makeAr, modelAr, year?.toString()].whereType<String>().join(' ');
  String get subtitle => [plate, if (vin != null) 'VIN ${vin!.substring(vin!.length - 6)}'].whereType<String>().join(' · ');
}
