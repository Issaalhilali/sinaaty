...
/// Socket.IO `/realtime` → subscribe `transport:{id}` → tick on any event. Falls back silently.
class TransportRealtimeImpl implements TransportRealtime {
  final String baseUrl; final TokenStore tokens; TransportRealtimeImpl({required this.baseUrl, required this.tokens});
  @override Stream<void> changes(String jobId) {
    ...
    c = StreamController<void>(onListen: () async {
      ...
      socket!.onConnect((_) => socket!.emit('subscribe', {'channel': 'transport:$jobId'}));
