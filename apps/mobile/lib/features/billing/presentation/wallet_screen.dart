...
class WalletScreen extends ConsumerWidget {
  ...
  @override Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AsyncResultView<List<Invoice>>(value: inv, onRetry: refresh, builder: (list) {
      ...
      if (list.isEmpty && ns.isEmpty) return EmptyState(icon: Icons.account_balance_wallet_outlined, title: l.walletEmptyTitle, body: l.walletEmptyBody, actionLabel: l.ptWarranties, onAction: () => context.push('/warranties'));
      ...
      return RefreshIndicator(onRefresh: refresh, child: ListView(padding: const EdgeInsets.fromLTRB(SinaatySpace.lg, SinaatySpace.sm, SinaatySpace.lg, 110), children: [
        if (due.isNotEmpty) ...[SealCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(l.due, style: Theme.of(context).textTheme.titleLarge?.copyWith(color: Colors.white)), Text('${due.length} ${l.invoices}${openNotes > 0 ? ' · $openNotes ${l.notes}' : ''}', style: Theme.of(context).textTheme.bodySmall?.copyWith(color: Colors.white.withValues(alpha: .75)))])), SealPill(l.amountHeld, icon: Icons.lock_outline)]),
        ...
        ])), const SizedBox(height: SinaatySpace.md), SectionTitle(l.due), SectionCard(padding: const EdgeInsets.symmetric(horizontal: SinaatySpace.sm), child: Column(children: [for (final i in due) invRow(i)])), const SizedBox(height: SinaatySpace.lg)],
