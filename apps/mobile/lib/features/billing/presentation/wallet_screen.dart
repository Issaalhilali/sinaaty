  ...
  @override Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AsyncResultView<List<Invoice>>(value: inv, onRetry: refresh, builder: (list) {
      ...
      return RefreshIndicator(onRefresh: refresh, child: ListView(padding: const EdgeInsets.all(SinaatySpace.lg), children: [
        if (due.isNotEmpty) ...[Text(l.due, style: Theme.of(context).textTheme.titleMedium), const SizedBox(height: SinaatySpace.sm), SectionCard(padding: const EdgeInsets.symmetric(horizontal: SinaatySpace.sm), child: Column(children: [for (final i in due) invRow(i)])), const SizedBox(height: SinaatySpace.lg)],
        if (ns.isNotEmpty) ...[Text(l.notes, style: Theme.of(context).textTheme.titleMedium), const SizedBox(height: SinaatySpace.sm), SectionCard(padding: const EdgeInsets.symmetric(horizontal: SinaatySpace.sm), child: Column(children: [for (final n in ns) AppListRow(icon: Icons.verified_outlined, title: l.noteNumber(n.number), subtitle: n.dueDate != null && n.isOpen ? l.dueOn(Fmt.date(n.dueDate!, locale: locale)) : Labels.noteStatus(l, n.status), trailing: Column(crossAxisAlignment: CrossAxisAlignment.end, mainAxisSize: MainAxisSize.min, children: [Text(Fmt.money(n.isOpen ? n.outstanding : n.amount, locale: locale), style: const TextStyle(fontWeight: FontWeight.w600)), StatusBadge(n.overdue ? l.overdue : Labels.noteStatus(l, n.status), tone: n.overdue ? BadgeTone.bad : n.isOpen ? BadgeTone.brass : BadgeTone.seal)]), onTap: () => context.push('/notes/${n.id}'))])), const SizedBox(height: SinaatySpace.lg)],
        if (rest.isNotEmpty) ...[Text(l.invoices, style: Theme.of(context).textTheme.titleMedium), const SizedBox(height: SinaatySpace.sm), SectionCard(padding: const EdgeInsets.symmetric(horizontal: SinaatySpace.sm), child: Column(children: [for (final i in rest) invRow(i)]))],
      ]));
    });
  }
}
