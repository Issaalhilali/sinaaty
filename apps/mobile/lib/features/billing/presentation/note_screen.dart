    ...
    return AppScaffold(title: n == null ? l.note : l.noteNumber(n.number),
      ...
      body: AsyncResultView<PromissoryNote>(value: v, onRetry: () => ref.invalidate(noteProvider(id)), builder: (note) => ListView(padding: const EdgeInsets.fromLTRB(SinaatySpace.lg, SinaatySpace.md, SinaatySpace.lg, 96), children: [
        SectionCard(glow: true, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(Fmt.meta([if (note.isOpen) l.outstanding, if (note.dueDate != null) l.dueOn(Fmt.date(note.dueDate!, locale: locale)), if (note.nafezReference != null) 'نافذ ${Fmt.ltr(note.nafezReference!)}']), style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant)),
        ...
        SectionCard(child: StatusTimeline(steps: [for (final e in note.events) TimelineStep(title: Labels.noteStatus(l, e.toStatus), subtitle: Fmt.meta([Fmt.dateTime(e.at, locale: locale), if (e.amountDelta != null) Fmt.money(e.amountDelta!, locale: locale), if (e.noteAr != null) e.noteAr!]), done: true, current: e == note.events.last)])),
