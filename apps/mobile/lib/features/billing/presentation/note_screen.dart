    ...
    return AppScaffold(title: n == null ? l.note : l.noteNumber(n.number),
      ...
      body: AsyncResultView<PromissoryNote>(value: v, onRetry: () => ref.invalidate(noteProvider(id)), builder: (note) => ListView(padding: const EdgeInsets.fromLTRB(SinaatySpace.lg, SinaatySpace.md, SinaatySpace.lg, 96), children: [
        SectionCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [Expanded(child: Text(Fmt.money(note.isOpen ? note.outstanding : note.amount, locale: locale), style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700))), StatusBadge(note.overdue ? l.overdue : Labels.noteStatus(l, note.status), tone: note.overdue ? BadgeTone.bad : note.isOpen ? BadgeTone.brass : BadgeTone.seal, icon: Icons.verified_outlined)]),
