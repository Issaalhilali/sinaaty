  ...
  @override Widget build(BuildContext context) {
    ...
    final body = AsyncResultView<List<WorkOrder>>(value: orders, onRetry: () => ref.invalidate(orgOrdersProvider), builder: (all) {
      ...
      return Column(children: [
        ...
        Expanded(child: list.isEmpty ? EmptyState(icon: Icons.inbox_outlined, title: l.wsNoOrders, body: l.wsNoOrdersBody, actionLabel: l.wsNewOrder, onAction: () => context.push('/ws/new')) : ListView(padding: EdgeInsets.fromLTRB(SinaatySpace.lg, SinaatySpace.md, SinaatySpace.lg, SinaatySpace.bottomClearance(context)), children: [SectionCard(padding: const EdgeInsets.symmetric(horizontal: SinaatySpace.sm), child: RowGroup(children: [for (final w in list) AppListRow(icon: Icons.directions_car_outlined, title: w.carLine, subtitle: Fmt.meta([w.titleAr ?? w.number, w.priced ? Fmt.money(w.total, locale: locale) : l.awaitingPricing]), trailing: StatusBadge(Labels.woStatus(l, w.status), tone: w.awaitingApproval ? BadgeTone.brass : w.isActive ? BadgeTone.seal : BadgeTone.plain), onTap: () => context.push('/ws/orders/${w.id}'))]))])),
      ]);
    });
