...
class _WorkshopOrderScreenState extends ConsumerState<WorkshopOrderScreen> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return AppScaffold(title: o?.number ?? l.workOrder, primaryAction: primary,
      moreItems: [if (o != null && o.status == 'inspecting') PopupMenuItem(value: 'item', child: Text(l.wsAddItem)), if (o != null && !const {'draft', 'closed', 'cancelled'}.contains(o.status)) PopupMenuItem(value: 'photo', child: Text(l.wsAddPhoto)), if (o != null && (ref.watch(featureFlagsProvider(ref.watch(currentOrgIdProvider))).value ?? FeatureFlags.allVisible).enabled(Flags.accidentReports)) PopupMenuItem(value: 'accident', child: Text(l.accOpen)), if (o != null && (ref.watch(featureFlagsProvider(ref.watch(currentOrgIdProvider))).value ?? FeatureFlags.allVisible).enabled(Flags.voiceToInvoice) && !ref.watch(voiceDeadProvider)) PopupMenuItem(value: 'voice', child: Text(l.voDictateItems))],
      ...
      body: AsyncResultView<WorkOrder>(value: wo, onRetry: _refresh, builder: (o) => RefreshIndicator(onRefresh: () async => _refresh(), child: ListView(padding: const EdgeInsets.fromLTRB(SinaatySpace.lg, SinaatySpace.sm, SinaatySpace.lg, 96), children: [
        SealCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(o.carLine, style: t.titleLarge?.copyWith(color: Colors.white)), Text(Fmt.meta([if (o.knowsCar) o.titleAr, o.number, Fmt.dateTime(o.createdAt, locale: locale)]), style: t.bodySmall?.copyWith(color: Colors.white.withValues(alpha: .75)))])), const SizedBox(width: 8), SealPill(Labels.woStatus(l, o.status))]),
