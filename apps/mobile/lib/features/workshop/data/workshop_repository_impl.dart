...
class WorkshopRepositoryImpl implements WorkshopRepository {
  ...
  @override Future<Result<List<OrgBrief>>> myOrgs() => _run(() async => (await api.dio.get<List<dynamic>>('/organizations/mine')).data!
      ...
      .map((d) => OrgBrief(id: d['id'] as String, nameAr: (d['name_ar'] ?? '') as String, type: d['type'] as String, status: d['status'] as String, role: d['role'] as String?, acceptingRequests: (d['accepting_requests'] as bool?) ?? true))
      .toList());
  ...
  @override Future<Result<bool>> setAvailability(String orgId, {required bool accepting}) => _run(() async => ((await api.dio.patch<Map<String, dynamic>>('/organizations/$orgId/availability', data: {'accepting_requests': accepting})).data!['accepting_requests']) as bool);
  @override Future<Result<List<OrgMember>>> members(String orgId) => _run(() async => ((await api.dio.get<List<dynamic>>('/organizations/$orgId/members')).data!).cast<Map<String, dynamic>>().map((m) => OrgMember(userId: m['userId'] as String, nameAr: m['fullNameAr'] as String?, phone: (m['phone'] ?? '') as String, role: m['role'] as String, isActive: (m['isActive'] as bool?) ?? true)).toList());
  @override Future<Result<void>> addMember(String orgId, {required String phone, required String role}) => _run(() async => (await api.dio.post<Map<String, dynamic>>('/organizations/$orgId/members', data: {'phone': phone, 'role': role})).data);
  @override Future<Result<void>> removeMember(String orgId, String userId) => _run(() async => (await api.dio.delete<Map<String, dynamic>>('/organizations/$orgId/members/$userId')).data);
  @override Future<Result<List<OrgServiceItem>>> serviceItems(String orgId) => _run(() async => ((await api.dio.get<List<dynamic>>('/organizations/$orgId/services')).data!).cast<Map<String, dynamic>>().map((x) => OrgServiceItem(id: x['id'] as String, nameAr: x['nameAr'] as String, itemType: x['itemType'] as String, unitPrice: x['unitPrice'] as String, warrantyDays: (x['warrantyDays'] as num).toInt())).toList());
  @override Future<Result<void>> addServiceItem(String orgId, {required String nameAr, required String unitPrice, String itemType = 'labor', int warrantyDays = 0}) => _run(() async => (await api.dio.post<Map<String, dynamic>>('/organizations/$orgId/services', data: {'name_ar': nameAr, 'unit_price': unitPrice, 'item_type': itemType, 'warranty_days': warrantyDays})).data);
  @override Future<Result<void>> removeServiceItem(String orgId, String id) => _run(() async => (await api.dio.delete<Map<String, dynamic>>('/organizations/$orgId/services/$id')).data);
