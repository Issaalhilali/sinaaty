...
class InboxScreen extends ConsumerWidget {
  ...
  @override Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AppScaffold(title: l.notifications, moreItems: [PopupMenuItem(value: 'read', child: Text(l.markAllRead))], onMore: (_) async { await ref.read(notificationsRepositoryProvider).markAllRead(); ref.invalidate(inboxProvider); ref.invalidate(unreadCountProvider); },
      body: AsyncResultView<List<AppNotification>>(value: v, onRetry: () => ref.invalidate(inboxProvider), builder: (list) => list.isEmpty ? EmptyState(icon: Icons.notifications_none, title: l.notificationsEmpty, body: l.notificationsEmptyBody) : RefreshIndicator(onRefresh: () async => ref.invalidate(inboxProvider), child: ListView.separated(padding: const EdgeInsets.all(SinaatySpace.lg), itemCount: list.length, separatorBuilder: (_, _) => const Divider(height: 1), itemBuilder: (_, i) { final n = list[i]; final route = routeFor(n.deepLink); return AppListRow(icon: n.read ? Icons.notifications_none : Icons.notifications_active_outlined, title: locale == 'ar' ? n.titleAr : (n.titleEn ?? n.titleAr), subtitle: '${locale == 'ar' ? n.bodyAr : (n.bodyEn ?? n.bodyAr)}\n${Fmt.dateTime(n.createdAt, locale: locale)}', onTap: route == null ? null : () async { await ref.read(notificationsRepositoryProvider).markRead(n.id); ref.invalidate(unreadCountProvider); if (context.mounted) await context.push(route); }); }))));
  }
}
