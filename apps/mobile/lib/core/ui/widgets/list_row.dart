...
class AppListRow extends StatelessWidget {
  final IconData? icon; final BrandGlyph? brandIcon; final String title; final String? subtitle; final Widget? trailing; final VoidCallback? onTap;
  const AppListRow({super.key, this.icon, this.brandIcon, required this.title, this.subtitle, this.trailing, this.onTap});
  @override Widget build(BuildContext context) {
    ...
    return InkWell(onTap: onTap, borderRadius: BorderRadius.circular(SinaatySpace.radius), child: Padding(padding: const EdgeInsets.symmetric(vertical: SinaatySpace.md, horizontal: SinaatySpace.sm), child: Row(children: [
      if (icon != null || brandIcon != null) ...[Container(width: 44, height: 44, alignment: Alignment.center, decoration: BoxDecoration(color: s.primaryContainer.withValues(alpha: .8), borderRadius: BorderRadius.circular(14)), child: brandIcon != null ? BrandIcon(brandIcon!, size: 24, color: s.onPrimaryContainer) : Icon(icon, color: s.onPrimaryContainer, size: 22)), const SizedBox(width: SinaatySpace.md)],
