...
**Current state (2026-08-17, after Step 0):** monorepo skeleton in place — Flutter template moved to `apps/mobile/`, root `package.json`/`pnpm-workspace.yaml`/`turbo.json`/`tsconfig.base.json`, `infra/docker/docker-compose.yml`, `.env.example`, ADRs 0001–0007, git initialized. `apps/api`, `apps/admin-web`, `packages/*` are still empty (Step 1+).
